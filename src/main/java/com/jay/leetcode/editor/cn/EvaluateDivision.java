...
// 
//
// Related Topics 深度优先搜索 广度优先搜索 并查集 图 数组 最短路 👍 969 👎 0
...
class Solution {
    ...
    public double[] calcEquation(List<List<String>> equations, double[] values, List<List<String>> queries) {
        ...
        for (int i = 0; i < equations.size(); i++) {
            ...
            graph.computeIfAbsent(s1, s -> new ArrayList<>()).add(new Cell(s2, values[i]));
            graph.computeIfAbsent(s2, s -> new ArrayList<>()).add(new Cell(s1, 1.0/values[i]));
        }
        ans = new double[queries.size()];
        Arrays.fill(ans, -1);
        Set<String> visited = new HashSet<>();
        for (int index=0;index<queries.size();index++) {
            dfs(queries.get(index).get(0), queries.get(index).get(1), 1.0, index, visited);
            visited.clear();
        }
        return ans;
    }

      private boolean dfs(String src, String dest, double cur, int index, Set<String> visited) {
          ...
          for (Cell cell : graph.getOrDefault(src, new ArrayList<>())) {
              if(dfs(cell.str, dest, cur * cell.value, index, visited)) {
                  return true;
              }
          }
          visited.remove(src);
      ...
      private class Cell {
        ...
        public Cell(String str, double value) {
            this.str = str;
            this.value = value;
        }
    }
